...
| Passive learning (`src/learning/sender-map.mjs`) | Turns accumulated operator corrections (`domain ≠ predicted_domain`) into a learned counterparty→domain prior that pre-seeds the routing hint (`effectiveHint`: learned → heuristic → unknown) | @unclenate | Built (feedback Phase 2a). Pure/offline; map holds labels only (no card content); routing-only this slice — classifier-prompt seeding is Phase 2b. No new runtime deps; `package.json` test script adds `tests/learning.test.mjs` |
