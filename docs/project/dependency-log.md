...
| Ollama (local LLM) | Vendor | @unclenate | Open (planned) | No offline/local-model provider option | 2026-06-24 | Planned provider; not yet present in `src/providers/`. Would let regression run with no external API key against a local model. |
| OpenAI API | Vendor | @unclenate | Open (planned) | Fewer LLM provider options behind the schema contract | 2026-06-24 | Planned provider; not yet present in `src/providers/`. |
