...
| -- | ---- | ---- | ---------- | ------ | ---------- | ----- | ------ |
| R-001 | Security | Loss of `KINETIC_TOKEN_ENCRYPTION_KEY`: the AES-256-GCM key lives only in `.env.local` (no backup, no secret manager). If lost or rotated, every `oauth_tokens.*_enc` blob becomes undecryptable — all stored Google/Microsoft access AND refresh tokens are permanently dead and every connected user must re-run the OAuth flow. | Med | High | Treat the key as a recovery asset: back it up out-of-band (password manager) before any reinstall/machine change. Document that re-OAuth is the only recovery path. Plan migration to a real secret store before multi-user. No automated rotation today. | @unclenate | Monitoring |
...
| R-006 | Security | **Single seeded user, no real auth yet**: v0.5 uses one hard-coded demo user (`00000000-...0001`, `KINETIC_DEMO_USER_ID`); full Supabase Auth is post-v0.5. There is no per-user isolation — anyone who can reach the server acts as the demo user and shares one OAuth token set. | High | Med | Acceptable for local-dev alpha (single operator, localhost only). RLS policies are already authored for the post-auth model. Blocking dependency: real auth must land before any multi-user or hosted deployment. | @unclenate | Open |
